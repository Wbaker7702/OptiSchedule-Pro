...
app.use(
  helmet({
    contentSecurityPolicy: false, // keeps life easier for inline demo UI
  })
);
...
<style>
  ...
  .btn{padding:10px 12px;border-radius:10px;border:1px solid rgba(148,163,184,.25);background:#111c33;color:#e5e7eb;cursor:pointer;font-weight:800}
    ...
    <div class="panel">
      <div class="card">
        ...
        <div class="row" style="margin-top:14px;">
          <button class="btn btnPrimary" onclick="fetch('/api/sim/black-friday',{method:'POST'}).then(()=>alert('Black Friday Mode Activated (demo)'));">
            Activate Black Friday Mode
          </button>
          <button class="btn" onclick="fetch('/api/sim/reset',{method:'POST'}).then(()=>alert('Simulation Reset'));">
            Reset Simulation
          </button>
        </div>
      </div>
...
app.post("/api/sim/black-friday", requireAuth, (req, res) => {
  ...
  res.json({ ok: true, mode: "BLACK_FRIDAY" });
});
